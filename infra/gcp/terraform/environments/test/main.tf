# OW TEST stack — a 1:1 copy of prod for axl-api-test's e2e suite
# (Slava, 2026-09-25: "полный стек как 1 в 1 на проде").
# Same module and tier as prod; its own VPC, Cloud SQL, service accounts,
# queues and scheduler jobs, all named open-wearables-test-*.
# Images come from PROD's registry (open-wearables-prod): one image per
# version, promoted, never rebuilt — so this env creates no registry.
