...
def schedule_stream_retry(user_id: str, activity_id: str, attempt: int) -> None:
    """Schedule attempt N with its configured backoff; deduped per attempt."""
    # Imported lazily to avoid a circular import (dispatcher -> tasks -> this module).
    ...
    dispatch_task(
        RegisteredTask.RETRY_STRAVA_STREAM_INGEST,
        kwargs={"user_id": user_id, "activity_id": activity_id, "attempt": attempt},
        countdown=countdown,
        dedup_key=f"strava_streams:{user_id}:{activity_id}:{attempt}",
    )
    log_structured(
        logger,
        "info",
