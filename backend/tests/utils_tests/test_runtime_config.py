...
def test_db_uri_prefers_database_url_override(monkeypatch) -> None:
...
def test_db_uri_supports_cloud_sql_socket_path(monkeypatch) -> None:
    ...
    assert settings.db_uri == (
        "postgresql+psycopg://socket-user:socket-password@/socket-db"
        "?host=%2Fcloudsql%2Fproject%3Aregion%3Ainstance"
    )


def test_redis_url_prefers_override(monkeypatch) -> None:
