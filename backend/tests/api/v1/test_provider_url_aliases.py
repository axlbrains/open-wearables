...
class TestUrlSlugMapping:
    ...
    def test_other_providers_ignore_the_flag(self, monkeypatch: pytest.MonkeyPatch) -> None:
        # A local config/.env can set the deprecated OURA_REDIRECT_URI, which short-circuits
        # the builder; clear it so this asserts on the path the builder produces.
        monkeypatch.setattr(settings, "oura_redirect_uri", None)
