...
class TestDisconnectWithSDKToken:
    ...
    def test_sdk_token_on_nonexistent_connection_succeeds(self, client: TestClient, db: Session) -> None:
        """Nothing to revoke is a no-op, not a 403.

        Fork divergence: disconnect is idempotent here (upstream answers 404). The caller's
        local state can be stale -- axl-api may still show the provider as connected after
        the row was cleaned up out-of-band -- and a successful disconnect is what lets the
        user re-authorize cleanly.
        """
        # Arrange
        user = UserFactory()
